    ...
    def __init__(self, sample_obs):
        ...
        for key, obs in sample_obs.items():
            if obs.ndim == 4 and obs.shape[-1] in (1, 3):
                ...
                with torch.no_grad():
                    n_flatten = cnn(obs.cpu().float().permute(0,3,1,2)).shape[1]
...
class PPOAgent(nn.Module):
    def __init__(self, action_shape, sample_obs_dict, state_shape = (7,)):
        super().__init__()
        self.feature_net = NatureCNN(sample_obs=sample_obs_dict)
...
def ppo_train_main(agent, cfg, env, test_env, post_process_fn=None, reward_fn=None, action_pre_process_fn=None, expert_buffer=None):
    ...
    logger = None
    video_recorder = VideoRecorder(Path(args.save_path))
    ...
    simple_obs_space = spaces.Dict({
        cam: spaces.Box(
            low=0, high=255,
            shape=(cfg.agent.image_size, cfg.agent.image_size, 3),
            dtype=np.uint8
        )
        for cam in cfg.agent.train_camera_name
    })
    ...
    sample_obs_dict = {k: v[0:1].to(device) for k, v in next_obs.items()}

    agent = PPOAgent(env.action_space.shape, sample_obs_dict).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            num_episodes = 0
            first_cam = cfg.agent.train_camera_name[0]
            video_recorder.init(eval_obs[first_cam][0].cpu().numpy(), enabled=True)
            done_num = 0
            for _ in range(args.num_eval):
                for _ in range(env.all_task_max_num):
                    with torch.no_grad():
                        eval_action = action_pre_process_fn(agent.get_action(eval_obs, deterministic=True))
                        eval_obs, eval_rew, done, eval_infos = test_env.step(eval_action)
                        eval_rew = torch.as_tensor(eval_rew, device=device)
                        eval_obs = post_process_fn(eval_obs)
                        video_recorder.record(eval_obs[first_cam][0].cpu().numpy())
                        ...
                        if done:
                            ...
                            eval_obs = test_env.reset()
                            eval_obs = post_process_fn(eval_obs)
                video_recorder.save(f'{iteration}.mp4')
            ...
            if logger is not None:
                ...
                logger.add_scalar("eval/eval_sucess", done_num/args.num_eval, global_step)
            if args.evaluate:
                break
        ...
        for step in range(0, args.num_steps):
            ...
            # print(global_step, infos["truncation"])
            if next_done.any():
                next_obs = env.reset()
                next_obs = post_process_fn(next_obs)
                done_indices = 0
            ...
            if "final_info" in infos:
                ...
                with torch.no_grad():
                    final_values[step, torch.arange(args.num_envs, device=device)[done_mask]] = agent.get_value(
                        infos["final_observation"]).view(-1)
