...
class PPOAgent(nn.Module):
    def __init__(self, action_shape, obs_shape = None, state_shape = (7,)):
        sample_obs = {"rgb": torch.zeros(1, *obs_shape)}
        super().__init__()
...
def ppo_train_main(agent, cfg, env, test_env, post_process_fn=None, reward_fn=None, action_pre_process_fn=None):
    ...
    # 5. ALGO Logic: Storage setup (初始化缓冲区)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            num_episodes = 0
            video_recorder.init(eval_obs["rgb"].cpu().numpy()[0], enabled=True)
            done_num = 0
            for _ in range(args.num_eval):
                for _ in range(env.all_task_max_num):
                    with torch.no_grad():
                        ...
                        eval_obs = post_process_fn(eval_obs)
                        video_recorder.record(eval_obs["rgb"].cpu().numpy()[0])
