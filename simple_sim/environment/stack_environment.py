...
class StackCanSimulation(SingleViewSimulation):
    def __init__(self, robot, env_info, has_renderer, *args, **kwargs):
        ...
        self.target_object = "blue_can"
        self.pick_reward_given = False
        self.stack_reward_given = False
        self.gripper_change_num = -2 # pick stack will change gripper once
        self.stack_threshold = 0.08
    ...
    def compute_dense_reward(self):
        ...
        stack_reward = 1 - np.tanh(5 * obj_to_goal_dist)
        reward += stack_reward * is_grasped
        print("stack_reward", stack_reward * is_grasped)
        reward += self.is_stack_done_from_sim() * self.sub_task_reward_scale
        print("stack_reward_given", self.stack_reward_given)
        if self.is_sucess():
    ...
    def is_sucess(self):
        if self.pick_reward_given and self.stack_reward_given:
            return True
        else:
            return False
    ...
    def is_stack_done_from_sim(self):
        moving_object_pose = self.env.sim.data.get_site_xpos(self.moving_object + "_center_site").copy()
        target_pose = self.env.sim.data.get_site_xpos(self.target_object + "_center_site").copy() 
        is_obj_stack = (np.linalg.norm(target_pose - moving_object_pose)<= self.stack_threshold)
        if is_obj_stack and not self.stack_reward_given:
            self.stack_reward_given = True
            return True
        else:
            return False
