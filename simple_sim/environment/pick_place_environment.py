...
class PickApplePlaceBowlSimulation(SingleViewSimulation):
    def __init__(self, robot, env_info, has_renderer, *args, **kwargs):
        ...
        self.moving_object = "apple"
        self.target_object = "bowl"
        self.pick_reward_given = False
        self.place_reward_given = False
        self.gripper_change_num = -2 # pick place will change gripper once
        self.place_threshold = 0.08
    ...
    def compute_dense_reward(self):
        ...
        print("place_reward", place_reward * is_grasped)
        reward += self.is_place_done_from_sim() * self.sub_task_reward_scale
        print("place_reward_given", self.place_reward_given)
        if self.is_sucess():
    ...
    def is_sucess(self):
        if self.pick_reward_given and self.place_reward_given:
            return True
        else:
            return False
    ...
    def is_place_done_from_sim(self):
        moving_object_pose = self.env.sim.data.get_site_xpos(self.moving_object + "_center_site").copy()
        target_pose = self.env.sim.data.get_site_xpos(self.target_object + "_center_site").copy() 
        is_obj_placed = (np.linalg.norm(target_pose - moving_object_pose)<= self.place_threshold)
        if is_obj_placed and not self.place_reward_given:
