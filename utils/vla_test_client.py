...
scene_image = np.random.randint(0, 256, (224, 224, 3), dtype=np.uint8)
hand_image = np.random.randint(0, 256, (224, 224, 3), dtype=np.uint8)
joint = np.random.rand(7,).astype(np.float32)
gripper = np.random.rand(1,).astype(np.float32)
task_prompt = "pick up the red cube"

def send_data(scene_image: np.ndarray, hand_image: np.ndarray, joint: np.ndarray, gripper: np.ndarray, task_prompt):
...
if __name__ == "__main__":
    action = send_data(scene_image, hand_image, joint, gripper, task_prompt)
    print(action)